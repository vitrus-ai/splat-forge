...
const App: React.FC = () => {
  ...
  React.useEffect(() => {
    ...
    const unlistenPromise = listen<{ paths: string[] }>("tauri://drag-drop", (e) => {
      if (e.payload.paths && e.payload.paths.length > 0) {
        ...
        if (path.endsWith(".splat") || path.endsWith(".ply")) {
    ...
    try {
      await invoke("run_reconstruction_pipeline", {
        args: {
          ...
          outputFilename: outputFilename.trim() || null,
          useDifix,
        },
      });
    } catch (e) {
  ...
  }, [inputPath, outputDir, outputFilename, isVideo]);
  ...
  return (
    <div className="h-screen flex flex-col splatforge-app bg-background text-foreground">
        ...
        <header className="splatforge-header border-b border-amber-500/20 px-4 py-2 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button
              ...
              className={cn("px-3 py-1.5 rounded-md text-sm font-medium transition-colors", activeTab === "worldgen" ? "bg-amber-500/20 text-amber-300 border border-amber-500/30" : "text-amber-500/50 hover:bg-white/5")}
            >
              World Gen
            </button>
            <button
              onClick={() => setActiveTab("setup")}
              className={cn("px-3 py-1.5 rounded-md text-sm font-medium transition-colors", activeTab === "setup" ? "bg-amber-500/20 text-amber-300 border border-amber-500/30" : "text-amber-500/50 hover:bg-white/5")}
            >
              Setup
            </button>
            <button
              ...
              className={cn("px-3 py-1.5 rounded-md text-sm font-medium transition-colors", (activeTab as string) === "preview" ? "bg-amber-500/20 text-amber-300 border border-amber-500/30" : "text-amber-500/50 hover:bg-white/5 disabled:opacity-30 disabled:cursor-not-allowed")}
            >
              Preview
            </button>
          </div>
      ...
      {activeTab === "worldgen" && !running && (
        <WorldGen onDatasetReady={(folderPath) => {
      ...
      {activeTab === "setup" && !running && (
        <div className="flex-1 min-h-0 overflow-auto p-6 flex flex-col md:flex-row justify-center gap-12 items-start max-w-6xl mx-auto w-full">
          {/* LEFT SIDE: Generation */}
          <div className="w-full max-w-md flex flex-col gap-4 relative z-10">
            <h2 className="text-lg font-medium text-center text-amber-400/90 mb-2">Create New Splat</h2>
            <Card className="splatforge-glass border-amber-500/20">
              <CardHeader>
                <CardTitle className="text-base text-amber-200/90">Input</CardTitle>
                <p className="text-xs text-amber-300/70 mt-1">
                  Choose a video to extract frames from, or select a folder containing already extracted images.
                </p>
              </CardHeader>
              <CardContent className="flex flex-col gap-3">
                <div className="flex flex-col sm:flex-row gap-3 items-center w-full">
                  <Button
                    ...
                    className="splatforge-glass-btn border-amber-500/30 text-amber-200/90 hover:bg-amber-500/15 flex-1 w-full"
                  >
                    <Video className="size-4 mr-2 shrink-0" />
                    <span className="truncate">Video (mp4/mov)</span>
                  </Button>
                  <span className="text-xs text-amber-500/50 font-medium uppercase tracking-wider shrink-0">or</span>
                  <Button
                    type="button"
                    variant="outline"
                    size="sm"
                  ...
                  >
                    ...
                    <span className="truncate">Images Folder</span>
                  </Button>
                </div>
                {inputPath && (
                  <p className="text-xs text-amber-300/70 truncate font-mono" title={inputPath}>
                    {isVideo ? "Video:" : "Folder:"} {inputPath}
                  </p>
                )}
              </CardContent>
            </Card>
            ...
            <Card className="splatforge-glass border-amber-500/20">
              <CardHeader>
                <CardTitle className="text-base text-amber-200/90">Output</CardTitle>
              </CardHeader>
              <CardContent className="flex flex-col gap-3">
                <div>
                  <Label className="text-xs text-amber-300/80">Output folder</Label>
                  <div className="flex gap-2 mt-1">
                    <Input
                      readOnly
                      value={outputDir}
                      placeholder="Choose folder…"
                      className="splatforge-glass-input font-mono text-xs border-amber-500/20 bg-black/30"
                    />
                    <Button
                      type="button"
                      variant="outline"
                      ...
                      className="splatforge-glass-btn border-amber-500/30 text-amber-200/90 hover:bg-amber-500/15"
                    >
                ...
                <div>
                  <Label className="text-xs text-amber-300/80">Filename (optional)</Label>
                  <Input
                    value={outputFilename}
                    onChange={(e) => setOutputFilename(e.target.value)}
                    placeholder={defaultOutputFilename()}
                    className="splatforge-glass-input font-mono text-xs mt-1 border-amber-500/20 bg-black/30"
                  />
                </div>
                ...
                <div className="flex items-center gap-2 mt-2 pt-2 border-t border-amber-500/20">
                  ...
                  />
                  <Label htmlFor="difix-toggle" className="text-sm text-amber-200/90 cursor-pointer">
                    Enable Difix3D+ Enhancement
                  </Label>
                </div>
            ...
            <div className="flex flex-col gap-2">
              <Button
                ...
                onClick={handleStart}
                className={cn(
                  "splatforge-generate-btn font-medium",
                  canGenerate
            ...
            {error && (
              <p className="text-amber-400 text-sm bg-amber-950/30 border border-amber-500/30 rounded-sm px-3 py-2">
                {error}
              </p>
            )}
          </div>
          ...
          {/* DIVIDER */}
          <div className="hidden md:block w-px bg-amber-500/20 self-stretch relative z-10" />

          {/* RIGHT SIDE: Open/Preview & Edit */}
          <div className="w-full max-w-md flex flex-col gap-4 relative z-10">
            <h2 className="text-lg font-medium text-center text-amber-400/90 mb-2">View & Edit Splats</h2>
            <Card className="splatforge-glass border-amber-500/20">
              <CardHeader>
                <CardTitle className="text-base text-amber-200/90">Open Existing</CardTitle>
                <p className="text-xs text-amber-300/70 mt-1">
                  Click below or drag and drop an existing .splat or .ply file to preview, navigate, and edit.
                </p>
              </CardHeader>
              <CardContent>
                <Button
                  type="button"
                  variant="outline"
                  onClick={handleOpenSplat}
                  className="splatforge-glass-btn border-amber-500/30 text-amber-200/90 hover:bg-amber-500/15 w-full h-32 flex flex-col gap-3 justify-center items-center"
                >
                  <FileUp className="size-8 opacity-70" />
                  <span className="font-medium text-amber-200/90">Open or drop a Splat (.splat/.ply)</span>
                </Button>
              </CardContent>
            </Card>
          </div>

          {/* Floating background splat */}
      ...
      {activeTab === "preview" && resultSplatPath && (
